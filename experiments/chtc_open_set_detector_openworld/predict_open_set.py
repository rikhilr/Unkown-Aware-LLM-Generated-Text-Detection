...
def predict(text, artifact_dir):
    ...
    llm_center = joblib.load(os.path.join(artifact_dir, "global_llm_deepsvdd_center.joblib"))
    triage_clf = joblib.load(os.path.join(artifact_dir, "stage1_triage_clf.joblib"))
    ...
    F = stage1_feature_matrix(X, human_llm_clf, known_model_clf, centers, radii, llm_center, energy_temperature)
    stage1_probs = triage_clf.predict_proba(F)[0]
    stage1_classes = list(triage_clf.classes_)
    stage1_pred = stage1_classes[int(stage1_probs.argmax())]

    result = {
        "stage1_prediction": stage1_pred,
        "stage1_probabilities": {c: float(p) for c, p in zip(stage1_classes, stage1_probs)},
    ...
    if stage1_pred == "llm":
        ...
        result["final_prediction"] = stage1_pred
    return result
