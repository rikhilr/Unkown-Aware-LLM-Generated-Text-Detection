...
python train_open_set_chtc.py \
  ...
  --known_models llama-chat,mpt,mpt-chat,chatgpt,gpt3,gpt4,cohere,cohere-chat \
  --val_unknown_models gpt2 \
  --test_unknown_models mistral,mistral-chat \
  ...
  --wandb_run_name "raid-htao-${CLUSTER:-local}-${PROCESS:-0}" \
  --wandb_log_every 5
TRAIN_EXIT_CODE=$?
set -e
