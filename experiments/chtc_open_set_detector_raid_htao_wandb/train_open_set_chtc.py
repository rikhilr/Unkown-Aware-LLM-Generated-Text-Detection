...
def collect_samples(args, wb):
    ...
    quotas_enabled = args.samples_per_label > 0
    ...
    log_print("Collecting RAID rows with stratified label quotas.")
    ...
    for raw in load_raid_stream(args):
        ...
        if not args.include_adversarial and s["attack"] not in {"none", "unknown", ""}:
            continue
        if quotas_enabled and counts[label] >= args.samples_per_label:
            # keep scanning until all target labels are filled or total cap is hit
            if seen % args.wandb_log_every == 0:
                pass
            if all(counts[l] >= args.samples_per_label for l in target_labels if l in counts or l in target_set):
                break
            continue
        ...
        if kept % max(1, args.wandb_log_every * 100) == 0:
            ...
            log_print(f"Collected {kept} rows after scanning {seen}. Counts: {dict(counts)}")

        if max_total is not None and kept >= max_total:
            break
        if quotas_enabled and all(counts[l] >= args.samples_per_label for l in target_labels):
            break

    log_print("Final label counts:")
    for k, v in counts.most_common():
        log_print(f"  {k}: {v}")
    log_print("Domain counts sample: " + str(domain_counts.most_common(15)))
    log_print("Attack counts sample: " + str(attack_counts.most_common(15)))
    wb.log({"data_collection/final_rows": len(rows), "data_collection/final_unique_labels": len(counts)})
    return rows
...
def choose_splits_by_source(samples, args):
    ...
    if not known:
        ...
        val_unknown = set(llm_sorted[n_known:n_known+n_val])
        test_unknown = set(llm_sorted[n_known+n_val:])
    ...
    for k, v in splits.items():
        log_print(f"  {k}: {len(v)}")
    return splits, known, val_unknown, test_unknown
...
def main():
    ...
    parser.add_argument("--known_models", default="llama-chat,mpt,mpt-chat,chatgpt,gpt3,gpt4,cohere,cohere-chat")
    parser.add_argument("--val_unknown_models", default="gpt2")
    parser.add_argument("--test_unknown_models", default="mistral,mistral-chat")
    ...
    splits, known, val_unknown, test_unknown = choose_splits_by_source(samples, args)
    ...
    X_binary_train = np.vstack([arrays["X_human_train"], arrays["X_llm_train"]])
    y_binary_train = np.array(["human"] * len(arrays["X_human_train"]) + ["llm"] * len(arrays["X_llm_train"]))
    ...
    llm_center = arrays["X_llm_train"].mean(axis=0)
    llm_train_dist = cosine_distances(arrays["X_llm_train"], llm_center.reshape(1, -1)).ravel()
    llm_val_dist = cosine_distances(arrays["X_llm_val_known"], llm_center.reshape(1, -1)).ravel()
    ...
    # Tune on val, evaluate on test. The returned thresholds/rejector are tuned from validation and reused for prediction.
    val_bundle = tune_and_eval_open_set(
        ...
        known_model_clf, centers, radii, args, wb, "open_set_val"
    )

    # Test uses thresholds/rejector learned on validation, not retuned.
    log_print("\n===== open_set_test using validation-tuned thresholds =====")
    Xtk, ytk = arrays["X_llm_test_known"], arrays["y_llm_test_known"]
    Xtu, ytu = arrays["X_llm_test_unknown"], arrays["y_llm_test_unknown"]
    known_pred_test = known_model_clf.predict(Xtk)
    rep_test = classification_report(ytk, known_pred_test, output_dict=True, zero_division=0)
    log_print("Known closed-set classification:\n" + classification_report(ytk, known_pred_test, zero_division=0))
    wb.log(classification_metrics_to_wandb("open_set_test/known_closed", rep_test))
    feats_k = ood_feature_matrix(Xtk, known_model_clf, centers, radii, args.energy_temperature)
    feats_u = ood_feature_matrix(Xtu, known_model_clf, centers, radii, args.energy_temperature)
    ...
    test_scores = {
        ...
        "ensemble_rejector": (rej_prob_k, rej_prob_u, val_bundle["rejector_threshold"]),
    }
    for method, (ks, us, th) in test_scores.items():
        ...
        known_correct_not_rej = ((known_pred_test == ytk) & (~known_rej)).mean()
        log_print(f"open_set_test/{method}: known_false_unknown={known_rej.mean():.4f}, unknown_detection={unk_rej.mean():.4f}, auroc={auroc:.4f}")
        wb.log({
            f"open_set_test/{method}/known_false_unknown_rate": float(known_rej.mean()),
            f"open_set_test/{method}/unknown_detection_rate": float(unk_rej.mean()),
            f"open_set_test/{method}/known_correct_not_rejected": float(known_correct_not_rej),
            f"open_set_test/{method}/distance_auroc": float(auroc),
        })
    ...
    joblib.dump(val_bundle["rejector"], os.path.join(args.output_dir, "ood_rejector_energy_deepsvdd.joblib"))
